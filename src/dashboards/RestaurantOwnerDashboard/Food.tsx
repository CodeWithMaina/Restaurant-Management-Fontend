...
export const Food = () => {
  ...
  const { data: restaurantMenuItems, isLoading, isError, refetch } = 
    menuItemApi.useFetchMenuItemByRestaurantIdQuery(restaurantId);
  ...
  const handleEdit = (id: number) => {
    const itemToEdit = restaurantMenuItems?.find(item => item.id === id);
    if (itemToEdit) {
      setEditingId(id);
      setIsModalOpen(true);
    }
  ...
  const handleToggleStatus = async (id: number) => {
    const item = restaurantMenuItems?.find(i => i.id === id);
    if (!item) return;
