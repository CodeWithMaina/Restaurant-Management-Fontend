...
export const orderApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    // Fetch order by ID
    fetchOrderById: builder.query<TOrder, number>({
      query: (id) => `orders/${id}`,
      providesTags: (result, error, id) => [{ type: "Order", id }],
    }),
    // Create order
    createOrder: builder.mutation<TOrder, Partial<TOrder>>({
      query: (newOrder) => ({
        url: "orders",
        method: "POST",
        body: newOrder,
    ...
    // Update order
    updateOrder: builder.mutation<TOrder, Partial<TOrder> & { id: number }>({
      query: ({ id, ...patch }) => ({
        url: `orders/${id}`,
        method: "PUT",
        body: patch,
